    ...
    K: Key,
{
    ...
    fn new(cluster: &Cluster, k: usize, p: usize) -> Self {
        Self {
    ...
    /// Simulate an access to a centralized MDS.
    async fn access_mds(&self, rpc: &Rpc) {
        let mds = if self.cluster.rank() == 0 { 1 } else { 0 };
        let mut req_buf = rpc.alloc_msgbuf(8);
        let mut resp_buf = rpc.alloc_msgbuf(8);
        unsafe { ptr::write(req_buf.as_mut_ptr() as *mut u64, self.cluster.rank() as u64) };

        let sess = RESOURCES.with(|res| {
            let res = unsafe { &*res.get() }.as_ref().unwrap();
            res.session_to(rpc, mds)
        });
        sess.request(RPC_FAKE_MDS_REQUEST, &req_buf, &mut resp_buf)
            .await;
    }
    ...
    pub async fn rpc_handler(&self, req: RequestHandle) -> MsgBuf {
        ...
        // Simulate centralized MDS access.
        self.access_mds(rpc).await;
        ...
        if req.req_type() == RPC_DATA_WRITE {
            ...
            let repl_index = (0..=self.p)
                ...
                .collect();
            self.repl_index.insert(hdr.key, repl_index);
            ...
            // Due to the (simulated) MDS access, we do not always need to perform replication.
            let need_repl = rand::thread_rng().gen_range(0..self.k);

            // Perform replication if I am primary.
            if primary == self.cluster.rank() && need_repl == 0 {
                ...
                let sessions = (0..self.p)
                    .map(|i| {
                        let node_id = (primary + i + 1) % self.cluster.size();
...
fn main() -> Result<()> {
    ...
    if cluster.rank() == 0 {
        log::info!("===== MDS baseline (simulated) =====");
    ...
    let objstore = Arc::new(<ObjStore<u64>>::new(&cluster, ec_config.k, ec_config.p));
