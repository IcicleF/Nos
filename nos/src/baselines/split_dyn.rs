...
struct ValueSplit {
    ...
    /// Nodes where the splits reside on.
    splits_at: Vec<usize>,
}

impl ValueSplit {
    fn new(len: usize, value: Box<[u8]>, n: usize) -> Self {
        Self {
            len,
            value,
            splits_at: vec![0; n],
    ...
    K: Key,
{
    ...
    fn insert(&self, key: K, split: Box<[u8]>, len: usize) {
        self.store
            .insert(key, ValueSplit::new(len, split, self.ec_config.p));
    }
    ...
    pub async fn rpc_handler(&self, req: RequestHandle) -> MsgBuf {
        ...
        if req.req_type() == RPC_DATA_WRITE && primary == self.cluster.rank() {
            ...
            // 4. Prepare sessions.
            // let sessions = {
            //     let mut sessions = Vec::with_capacity(stripe_width - 1);
            //     for i in 1..self.cluster.size() {
            //         let node_id = (self.cluster.rank() + i) % self.cluster.size();
            //         assert_ne!(node_id, self.cluster.rank(), "replicating to self");
            ...
            //             sessions.push(res.session_to(rpc, node_id))
            //         });
            //         if sessions.len() == stripe_width - 1 {
            //             break;
            //         }
            //     }
            //     sessions
        ...
        if req.req_type() == RPC_DATA_READ {
            ...
            if let Some(my_split) = my_split {
                ...
                // Simulate using the split map data.
                std::hint::black_box(&my_split.splits_at);
                ...
                let req_bufs = (0..self.ec_config.k)
                    .map(|_| {
                        let msgbuf = rpc.alloc_msgbuf(<ObjMeta<K>>::len());
                        unsafe {
                            ptr::write_unaligned(
                                msgbuf.as_ptr() as *mut ObjMeta<K>,
                ...
                let mut read_futs = Vec::with_capacity(self.ec_config.k);
                let mut split_src = Vec::with_capacity(self.ec_config.k);
                ...
                for (i, sess) in sessions.iter().enumerate() {
                    ...
                    if node_id == self.cluster.rank() {
                        ...
                        let nested_req_buf = &req_bufs[split_src.len()];
                    ...
                    split_src.push(i);
                    if split_src.len() == self.ec_config.k {
                        break;
                    }
                }
                assert!(
                    split_src.len() == self.ec_config.k,
                    "not enough splits to reconstruct an object"
                );

                // 4. Concurrently wait for all requests to finish.
                join_all(read_futs).await;

                // If necessary, perform recovery.
                if split_src.iter().any(|&i| i >= self.ec_config.k) {
                    ...
                    // Perform recovery.
                    self.ec_config.recover(&mut data, &parity, &split_src);
                }
