...
use nos::ctrl::*;
use nos::ec::EcConfig;
use nos::objstore::{AtomicVersion, ObjMeta, ObjStoreStats, RpcRes, Version};
use nos::rpc_types::*;
...
impl VerObject {
    fn new(obj: Object, version: Version) -> Self {
        Self { obj, version }
    }
}

/// An object handed-off to me.
#[derive(Clone)]
struct HandoffObject<K>
where
    K: Key,
{
    okey: K,
    ...
    K: Key,
{
    ...
    store: DashMap<K, VerObject, RandomState>,
    /// Replication index.
    repl_index: DashMap<K, Vec<usize>, RandomState>,
    /// Handoff area.
    handoff: DashMap<usize, Vec<HandoffObject<K>>, RandomState>,
    /// Version.
    ...
    K: Key,
{
    ...
    fn new(cluster: &Cluster, p: usize) -> Self {
        Self {
            ...
            handoff: DashMap::with_hasher(RandomState::default()),
            version: AtomicVersion::new(0),
    ...
    #[inline]
    fn insert(&self, key: K, primary: usize, value_addr: *const u8, len: usize, ver: Version) {
        let entry = self.store.entry(key);
        if let dashmap::Entry::Occupied(ref entry) = entry {
            if entry.get().version >= ver {
                // The object is already in the store and has a newer version.
                return;
            }
        }
        ...
        let obj = if primary == self.cluster.rank() {
            Object::Primary(value)
        } else {
            Object::Replica(value)
        };
        entry.insert(VerObject::new(obj, ver));
    }
    ...
    /// Run a collector that periodically collects the handoff objects, and
    /// determine whether they can be returned to their owner nodes.
    ///
    /// **NOTE:** this method is intended to be ran in a separate thread.
    pub fn collector(&self, rpc: Pin<Box<Rpc>>, sessions: Vec<SessId>) {
        const COLLECTION_INTERVAL: Duration = Duration::from_millis(1);
        const MAXIMUM_RETURN: usize = 1 << 20;

        let mut handback_start = None;
        ...
        while !self.is_halted() {
            ...
            // Collect the handoff objects.
            let mut count = 0;
            let nodes = {
                let mut nodes = (0..self.cluster.size())
                    .filter(|&i| i != self.cluster.rank())
                    .collect::<Vec<_>>();
            ...
            for owner in nodes {
                // Return to host if it is not slow.
                let handoff = if !self.slow_down[owner].load(Ordering::Relaxed) {
                    match self.handoff.get_mut(&owner) {
                        None => continue,
                        Some(mut handoff) => {
                            if handoff.is_empty() {
                                continue;
                            }
                            let to_take = handoff.len().min(MAXIMUM_RETURN - count);
                            handoff.drain(..to_take).collect::<Vec<_>>()
                        }
                    }
                } else {
                    continue;
                ...
                count += handoff.len();
                if handback_start.is_none() {
                    log::info!("Handoff returning started");
                    handback_start = Some(Instant::now());
                }

                let owner = rpc.get_session(sessions[owner]).unwrap();
                const CONCURRENCY: usize = 8;

                let mut resp_bufs: [MsgBuf; CONCURRENCY] = array::from_fn(|_| rpc.alloc_msgbuf(16));
                for objs in handoff.chunks(CONCURRENCY) {
                    let mut req_bufs = objs
                        .iter()
                        .map(|obj| {
                            let mut buf = rpc.alloc_msgbuf(ObjMeta::<K>::len() + obj.ovalue.len());
                            unsafe {
                                ptr::write(
                                    buf.as_mut_ptr() as *mut ObjMeta<K>,
                                    ObjMeta::new(obj.okey, obj.ovalue.len(), obj.version),
                                );
                                ptr::copy_nonoverlapping(
                                    obj.ovalue.as_ptr(),
                                    buf.as_mut_ptr().add(ObjMeta::<K>::len()),
                                    obj.ovalue.len(),
                                );
                            }
                            buf
                    ...
                    let reqs = req_bufs
                        .iter_mut()
                        .zip(resp_bufs.iter_mut())
                        .map(|(req_buf, resp_buf)| owner.request(RPC_DATA_WRITE, req_buf, resp_buf))
                        .collect::<Vec<_>>();
                    block_on(join_all(reqs));
                }

                // Rate limit (load balancing).
                if count >= MAXIMUM_RETURN {
                    break;
                }
            }

            last_empty = count == 0;
            if count == 0 && handback_start.is_some() {
                log::info!(
                    "Handoff returning finished in {:?}",
                    handback_start.unwrap().elapsed()
                );
                handback_start = None;
            }
        }
    ...
    pub async fn rpc_handler(&self, req: RequestHandle) -> MsgBuf {
        ...
        if req.req_type() == RPC_DATA_WRITE {
            ...
            let repl_index = (0..=self.p)
                ...
                .collect::<Vec<_>>();
            let version = if primary == self.cluster.rank() {
                // This is a primary object.
                self.version.fetch_add(1, Ordering::SeqCst)
            } else {
                // This is a replica object.
                hdr.ver
            };

            if repl_index.contains(&self.cluster.rank()) {
                // This object belongs to me.
                // Insert the object's replication index.
                self.repl_index.insert(hdr.key, repl_index.clone());

                // Insert the object into its store.
                self.insert(
                    hdr.key,
                    primary,
                ...
                // This is a handoff.
                assert!(
                    primary != self.cluster.rank(),
                    "primary object should never be handed-off (node {}, key {})",
                    self.cluster.rank(),
                    hdr.key
                );
                ...
                let ovalue = {
                    let mut value = Vec::with_capacity(hdr.len as usize);
                    unsafe {
                        ...
                        value.set_len(hdr.len as usize);
                    };
                    value
                };
                let obj = HandoffObject {
                    okey: hdr.key,
                    ovalue,
                    version: hdr.ver,
                };

                // Push the object into the handoff area.
                match self.handoff.entry(primary) {
            ...
            if primary == self.cluster.rank() {
                ...
                // 2. Prepare sessions.
                let mut targets = Vec::with_capacity(self.p);
                for i in 1..self.cluster.size() {
                    let node_id = (primary + i) % self.cluster.size();
                    if self.cluster[node_id].is_alive()
                        && !self.slow_down[node_id].load(Ordering::Relaxed)
                    {
                        targets.push(node_id);
                        if targets.len() == self.p {
                            break;
                        }
                    }
                }
                assert!(targets.len() <= self.p);
                if targets.len() < self.p {
                    for i in 1..self.cluster.size() {
                        let node_id = (primary + i) % self.cluster.size();
                        if self.cluster[node_id].is_alive() && !targets.contains(&node_id) {
                            targets.push(node_id);
                            if targets.len() == self.p {
                                break;
                            }
...
fn main() -> Result<()> {
    ...
    if cluster.rank() == 0 {
        log::info!("===== Repl+Handoff baseline =====");
    ...
    for id in 1..=nthreads.ec {
        let remote_id = ((id % nthreads.rpc) + 1) as RpcId;
        ...
        listener_handles.push(thread::spawn(move || {
            ...
            // Prepare the RPC
            let rpc = Rpc::new(&nexus, (nthreads.rpc + id) as RpcId, NIC_NAME, NIC_PHYPORT);
            let mut sessions = Vec::with_capacity(cluster.size());
            for i in 0..cluster.size() {
                sessions.push(if i == cluster.rank() {
                    0
                } else {
                    let uri = cluster[i].uri();
                    let sess = rpc.create_session(uri, remote_id);
                    block_on(sess.connect());
                    assert!(
                        sess.is_connected(),
            ...
            objstore.collector(rpc, sessions);
        }));
    }
