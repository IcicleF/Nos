...
mod avltree {
    ...
    impl AvlNode {
        ...
        fn update_height(&mut self) {
            let lheight = if let Some(left) = NonNull::new(self.left) {
                unsafe { left.as_ref().height }
            } else {
                0
            };
            let rheight = if let Some(right) = NonNull::new(self.right) {
                unsafe { right.as_ref().height }
            } else {
                0
            };
    ...
    impl AvlTree {
        ...
        pub fn insert(&mut self, item: *mut Value) -> NonNull<AvlNode> {
            ...
            while let Some(mut node) = NonNull::new(unsafe { *nodep }) {
...
{
    #[inline]
    fn new(meta: ObjMeta<K>, stripe_id: usize, xid: Xid, committed: Xid, offset: usize) -> Self {
        Self {
            meta,
            stripe_id,
            xid,
...
/// Generate all m-combinations of the given n elements.
fn comb<T: Copy>(elems: &[T], m: usize) -> Vec<Vec<T>> {
    ...
    fn backtrack<T: Copy>(
        ...
        for i in start..elems.len() {
            combo.push(elems[i]);
            backtrack(i + 1, elems, m, combo, result);
            combo.pop();
        }
    }

    if m <= elems.len() {
        backtrack(0, elems, m, &mut combo, &mut result);
    }

    result
}

/// Maximum number of allowed slowdown nodes.
const MAX_SLOWDOWN_NODES: usize = 2;

/// Generate the stripe map like that in MemEC.
/// Return a vector of vectors of node IDs in a stripe.
fn generate_stripe_map(ec_config: &EcConfig, cluster_size: usize) -> Vec<Vec<usize>> {
    let mut ret = Vec::new();
    for start in 0..cluster_size {
        let mut stripe = Vec::new();

        // Push the nodes for data chunks.
        for i in 0..ec_config.k {
            stripe.push((start + i) % cluster_size);
        }

        // Generate combinations of parity chunks.
        let parity_cands = (0..(ec_config.p + MAX_SLOWDOWN_NODES))
            .map(|i| (start + ec_config.k + i) % cluster_size)
            .collect::<Vec<_>>();
        for parities in comb(&parity_cands, ec_config.p) {
            let mut stripe = stripe.clone();
            stripe.extend(parities);
            ret.push(stripe);
        }
    }
    ret
...
/// Filter the given stripe map, retaining those stripes that contain the given node.
/// The new stripe map is a map from stripe ID to the nodes in the stripe.
fn filter_stripe_map(stripe_map: Vec<Vec<usize>>, node: usize) -> HashMap<usize, Vec<usize>> {
    let mut ret = HashMap::with_hasher(RandomState::new());
    for (i, stripe) in stripe_map.iter().enumerate() {
        if stripe.contains(&node) {
            ret.insert(i, stripe.clone());
        }
    }
    ret
}

/// Location of a primary object.
    ...
    K: Key,
{
    ...
    fn new(cluster: &Cluster, ec_config: &EcConfig) -> Self {
        let stripe_map = filter_stripe_map(
            generate_stripe_map(ec_config, cluster.size()),
            cluster.rank(),
        );
        let valuebuf = stripe_map
            .keys()
            .map(|&k| {
                (
                    k,
            ...
            .collect();
        let alloc = stripe_map
            .keys()
            .map(|&k| (k, ecalloc::EcAlloc::new(Self::VIRTUAL_BUFFER_SIZE)))
            .collect();
        ...
        Self {
            ...
            primarybuf: DashMap::with_hasher(RandomState::default()),
            valuebuf,
            alloc,
    ...
    #[inline]
    fn insert(&self, stripe_id: usize, key: K, value_addr: *const u8, len: usize) -> usize {
        ...
        assert!(
            self.stripe_map.contains_key(&stripe_id),
            "stripe {} not found in stripe map",
            stripe_id
        );
        ...
        let offset = self.alloc[&stripe_id].alloc(len).expect("no memory");
        ...
        for i in first_byte_in..=last_byte_in {
            let mut value = self.valuebuf[&stripe_id]
                .entry(i)
    ...
    /// in its stripe.
    #[inline]
    fn update(&self, stripe_id: usize, value_addr: *const u8, offset: usize, len: usize) {
        assert!(
            self.stripe_map.contains_key(&stripe_id),
            "stripe {} not found in stripe map",
            stripe_id
    ...
    fn get(&self, key: K) -> Option<Vec<u8>> {
        ...
        let mut copied = 0;
        for i in first_byte_in..=last_byte_in {
            let value = self.valuebuf[&stripe_id]
                .entry(i)
                .or_insert_with(|| [0; ALLOC_UNIT]);
    ...
    /// Pick a stripe for a primary object.
    fn pick_stripe(&self) -> usize {
        // Find all stripes that contain this node as a primary.
        let cands = self
            .stripe_map
            .iter()
            .filter(|(_, v)| {
                v.iter()
                    .take(self.ec_config.k)
                    .any(|&n| n == self.cluster.rank())
            })
            .collect::<Vec<_>>();

        // Try to filter out all slow nodes.
        let fast_cands = cands
            .iter()
            .filter(|(_, v)| {
                v.iter()
                    .skip(self.ec_config.k)
                    .all(|&n| !self.slow_down[n].load(Ordering::Relaxed))
            })
            .collect::<Vec<_>>();

        if !fast_cands.is_empty() {
            let idx = rand::thread_rng().gen_range(0..fast_cands.len());
            *fast_cands[idx].0
        } else {
            let idx = rand::thread_rng().gen_range(0..cands.len());
            *cands[idx].0
        }
    }
    ...
    pub async fn rpc_handler(&self, req: RequestHandle) -> MsgBuf {
        ...
        if req.req_type() == RPC_DATA_WRITE {
            ...
            if primary == self.cluster.rank() {
                ...
                // Find a stripe for this object.
                let stripe_id: usize = self.pick_stripe();
                ...
                let (offset, diff) = match self.get(hdr.key) {
                    Some(val) => {
                        let PrimaryLocation {
                            stripe_id, offset, ..
                        } = *self.index.get(&hdr.key).unwrap().value();
                ...
                let sessions = (0..self.ec_config.p)
                    .map(|i| {
                        let node_id = self.stripe_map[&stripe_id][self.ec_config.k + i];
                ...
                for i in 0..self.ec_config.p {
                    ...
                    unsafe {
                        ptr::write_unaligned(
                            ...
                            RaidRpcRequest::new(hdr, stripe_id, xid, committed, offset),
                        )
                    };
                ...
                // Extract the diff and put it into the waiting queue
                let diff = unsafe {
...
mod tests {
    ...
    fn test_comb() {
        ...
        let combinations = super::comb(&elems, 2);
        assert_eq!(combinations.len(), 6);
        assert_eq!(combinations[0], vec![1, 2]);
        assert_eq!(combinations[1], vec![1, 3]);
        assert_eq!(combinations[2], vec![1, 4]);
        assert_eq!(combinations[3], vec![2, 3]);
        assert_eq!(combinations[4], vec![2, 4]);
        assert_eq!(combinations[5], vec![3, 4]);
    }
}
